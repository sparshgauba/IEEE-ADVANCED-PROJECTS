...
void RF24::setAutoAck(bool enable)
{
  ...
  if (enable)
  {
    write_register(EN_AA, 0x3F, 1);
  }
  else
  {
    write_register(EN_AA, 0x00, 1);
  }
    // TODO: END HERE
}
...
void RF24::setPALevel(uint8_t level)
{
  ...
  read_register(RF_SETUP, &curr_state, 1);
  curr_state &= 0xF9;
  uint8_t new_state = curr_state | bit_mask;
  write_register(RF_SETUP, &new_state, 1);
...
void RF24::setCRCLength(rf24_crclength_e length)
{
    ...
    uint8_t curr_state;
    read_register(CONFIG, &curr_state, 1);
    curr_state &= 0xF3; //XXXX00XX
    uint8_t new_state = curr_state;

    if (length == RF24_CRC_8)
      new_state |= 0x08;
    else if (length == RF24_CRC_16)
      new_state |= 0x0C;
    // No need for checking CRC_DISABLED
    write_register(CONFIG, &new_state, 1);
...
{
    uint8_t bit_mask = (delay << 4) | count;
    write_register (SETUP_RETR, &bit_mask, 1);
}
